...
class AmclNode
{
  ...
  private:
    ...
    int m_initial_force_update_count;
...
int main(int argc, char** argv)
{
  ...
  ros::NodeHandle nh;
  //std::cout<<" enter amcl node .............................................................." <<std::endl;
...
{
  std::cout<<" ###################### new amcl node ######################" <<std::endl;
  m_initial_force_update_count = 0;
  ...
  private_nh_.param("min_particles", min_particles_, 100);//最小粒子数
  private_nh_.param("max_particles", max_particles_, 5000);//最大粒子
  ...
  std::cout<<"laser_likelihood_max_dist: " << laser_likelihood_max_dist_ <<std::endl;
  ...
  ROS_INFO("laser_model_type: %s.",tmp_model_type.c_str());
  ...
  ROS_INFO("odom_model_type: %s.",tmp_model_type.c_str());
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////
  laser_scan_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(nh_, scan_topic_, 1000);
  
  //tf::MessageFilter，顾名思义是tf下的消息过滤器
  ...
  //laser_sub_ = nh_.subscribe(scan_topic_, 10, &AmclNode::laserReceived, this);
  ...
  //m_force_update = false; //initial code
  // ROS_INFO("set m_force_update true ............");
  m_force_update = true;
  ...
  ROS_INFO("AmclNode .................................... constructor over.",global_frame_id_.c_str());
...
void AmclNode::updatePoseFromServer()
{
  ...
  ROS_INFO("initial pose to server. x: %.3f, y: %.3f, yaw: %.3f", init_pose_[0], init_pose_[1],init_pose_[2] );
  ROS_INFO("initial pose's cov. x: %.3f, y: %.3f, yaw: %.3f", init_cov_[0], init_cov_[1],init_cov_[2] );	
}
...
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
  ...
  // Initialize the filter
  //get initial poze and initialize thefilter
  updatePoseFromServer();
                      ...
                      const ros::Time& t, const std::string& f)
{
  ...
  {
    ROS_WARN("1111 Failed to compute odom pose, skipping scan (%s)", e.what());
    return false;
  }
...
{
  ROS_INFO(" call amcl once ......................................................................");


  //return;


  m_initial_force_update_count++;
  if(m_initial_force_update_count>100)
  {
    m_initial_force_update_count = 100;
  }
  //坐标系名称
  ...
  if(frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end())//找到当前雷达坐标系
  {
    ...
    try
    {
      this->tf_->transform(ident, laser_pose, base_frame_id_);//雷达坐标转换到base_link坐标系中
    }
    catch(tf2::TransformException& e)
    {
    ...
    laser_pose_v.v[0] = laser_pose.pose.position.x;//在雷达在base_link中的位置
    laser_pose_v.v[1] = laser_pose.pose.position.y;
    // laser mounting angle gets computed later -> set to 0 here!
    laser_pose_v.v[2] = 0;
    lasers_[laser_index]->SetLaserPose(laser_pose_v);
    ROS_INFO("Received laser's pose wrt robot(base_link): %.3f %.3f %.3f",
              laser_pose_v.v[0],
              laser_pose_v.v[1],
              laser_pose_v.v[2]);

    frame_to_laser_[laser_scan_frame_id] = laser_index;//记录雷达信息(有多个雷达)
  } 
  else 
  {
    // we have the laser pose, retrieve laser index
    laser_index = frame_to_laser_[laser_scan_frame_id];//存储雷达
  }

  // Where was the robot when this scan was taken?
  //机器人在哪
  // 获得base在激光雷达扫描时候相对于odom的相对位姿
  pf_vector_t pose;//把base_link中心点 转换到 里程计坐标系中的位置
  ...
  ROS_INFO("get robot pose in odom: %.3f %.3f %.3f",
              pose.v[0],
              pose.v[1],
              pose.v[2]);


  pf_vector_t delta = pf_vector_zero();//0向量

  if(pf_init_)//filter is initialized
  {
    // Compute change in pose
    //delta = pf_vector_coord_sub(pose, pf_odom_pose_);
    ///////////////////////////////////////////////////////////////////////////////////////
    //获取里程计相对pose的变化 delta
    //////////////////////////////////////////////////////////////////////////////////////
    //pf_odom_pose_ 初始值为0
    delta.v[0] = pose.v[0] - pf_odom_pose_.v[0];
    delta.v[1] = pose.v[1] - pf_odom_pose_.v[1];
    delta.v[2] = angle_diff(pose.v[2], pf_odom_pose_.v[2]);//角度的变化

    // See if we should update the filter
    bool update = fabs(delta.v[0]) > d_thresh_ ||
                  fabs(delta.v[1]) > d_thresh_ ||
                  fabs(delta.v[2]) > a_thresh_; //当变换值大于阀值就更新
...
///////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ROS_INFO("force_update: %d ,sample_count: %d , min_particles: %d",m_force_update?1:0,tmp->sample_count,min_particles_);
    //ROS_INFO("tmp->sample_count  ..............................%d",tmp->sample_count);
    //ROS_INFO("min_particles_  .................................%d",min_particles_);
    // if (tmp->sample_count < (min_particles_ + 500) && m_force_update == true)   //add by yuanguobin
    // {
    ...
    //   //else
    //   //{
    //     ROS_INFO("m_force_update set false ..........");
        
    //     m_force_update=false;
    //   //}
    //  //autoReInitialFlag = true;
    // }
    // if(m_initial_force_update_count >= 3)
    // {
    //   m_force_update=false;
    // }
    
    //当robot位置不发生变化时
    //m_force_update = fabs(delta.v[0]) < 0.2 || fabs(delta.v[1]) <0.2;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(!pf_init_)//如果是第一次,滤波器还没有初始化
  {
    // Pose at last filter update
    pf_odom_pose_ = pose;//机器pose(来自里程计)赋值给pf,当做初值使用

    // Filter is now initialized
    pf_init_ = true;

    // Should update sensor data
    for(unsigned int i=0; i < lasers_update_.size(); i++)//不是一台雷达
      lasers_update_[i] = true;
  ...
  else if(pf_init_ && lasers_update_[laser_index])//如果不是第一次//如果已经初始化并需要更新则更新运动模型
  {
    //printf("pose\n");
    ...
    ROS_INFO("UpdateAction getting robot pose in odom: %.3f %.3f %.3f",
              pose.v[0],
              pose.v[1],
              pose.v[2]);
    ROS_INFO("UpdateAction getting pf_odom_pose_.....: %.3f %.3f %.3f",
              pf_odom_pose_.v[0],
              pf_odom_pose_.v[1],
              pf_odom_pose_.v[2]);
    ROS_INFO("UpdateAction getting delta vaue(pose - pf_odom_pose): %.3f %.3f %.3f",
          delta.v[0],
          delta.v[1],
          delta.v[2]);
  ...
  if(lasers_update_[laser_index])
  {
    ...
    AMCLLaserData ldata; //确定当前的雷达
    ldata.sensor = lasers_[laser_index];
    ldata.range_count = laser_scan->ranges.size();

    ////////依据雷达安装情况,转换雷达max,min increment angle 到base_link中///////////////////////////////////////////////////////////////////////////
  ...
  ////////////////////////////////////////////////////////////////////////////////////
    ...
    ldata.ranges = new double[ldata.range_count][2];//2维数组
    ROS_ASSERT(ldata.ranges);
    for(int i=0;i<ldata.range_count;i++)
    {
      // amcl doesn't (yet) have a concept of min range.  So we'll map short
      // readings to max range.
      if(laser_scan->ranges[i] <= range_min)//激光雷达传上来的数据只标记了最大值最小值，但是没做处理，直接将原始数据传上来，
        ldata.ranges[i][0] = ldata.range_max;//这里将最小值当最大值处理，因为在类似likelihood_field模型中，会直接将最大值丢弃
      else
    ...
    //该函数需要传入相应模型的处理函数，这里所有的测量模型在《概率机器人》的第六章有详细讲,后面只针对自己使用的likelihood_field模型讲一下
    ...
    /////////////////////////////////////////////////////////////////////////////
    pf_odom_pose_ = pose;//使用里程计里的pose更新pf的pose初值
    ////////////////////////////////////////////////////////////////////////////
    ...
    if (!m_force_update)//如果不强力更新的话,发布粒子云
    {
  ...
  if(resampled || force_publication)//如果重采样
  {
    ...
    {
      double weight;//权重
      pf_vector_t pose_mean;//均值
      pf_matrix_t pose_cov; //协方差阵
      ...
      if(hyps[hyp_count].weight > max_weight)//粒子权重大于最大值时,更新
      {
    ...
    if(max_weight > 0.0)//最大权重大于0
    {
      ...
      tf2::Quaternion q;
      q.setRPY(0, 0, hyps[max_weight_hyp].pf_pose_mean.v[2]);//航向角
       ...
       */
      //发布最优粒子(最佳位置)
      ROS_INFO("amcl_pose: %6.3f %6.3f %6.3f ", p.pose.pose.position.x, p.pose.pose.position.y,p.pose.pose.position.z);
      pose_pub_.publish(p);
      ...
      try
      {
        ...
        tf2::Transform tmp_tf(q, tf2::Vector3(hyps[max_weight_hyp].pf_pose_mean.v[0],
                                              hyps[max_weight_hyp].pf_pose_mean.v[1],
                                              0.0));
        ////map坐标系原点在base_link坐标系下的表示
        geometry_msgs::PoseStamped tmp_tf_stamped;
        tmp_tf_stamped.header.frame_id = base_frame_id_;
        tmp_tf_stamped.header.stamp = laser_scan->header.stamp;
        tf2::toMsg(tmp_tf.inverse(), tmp_tf_stamped.pose);

        ROS_INFO("tmp_tf_stamped(tmp_tf.inverse) pose: %6.3f %6.3f %6.3f",
            tmp_tf_stamped.pose.position.x,
            tmp_tf_stamped.pose.position.y,
            tmp_tf_stamped.pose.position.z);

        ////map坐标系原点在 odom坐标系下的表示
        this->tf_->transform(tmp_tf_stamped, odom_to_map, odom_frame_id_);//获取最优粒子在里程计坐标的位置


        ROS_INFO("odom_to_map pose: %6.3f %6.3f %6.3f",
            odom_to_map.pose.position.x,
            odom_to_map.pose.position.y,
            odom_to_map.pose.position.z);
...
{
  ROS_INFO("get the initial pose.........: %f, %f, %f ",msg->pose.pose.position.x,msg->pose.pose.position.y,msg->pose.pose.position.z);
