...
public:
  ...
  bool isGoalReached(bool ischarging, LocalPlannerUtil* planner_util,
      OdometryHelperRos& odom_helper,
      const geometry_msgs::PoseStamped& global_pose,double &xy_limited_tolerance);
  ...
  bool computeVelocityCommandsStopRotate(geometry_msgs::Twist& cmd_vel,
      ...
      boost::function<bool (Eigen::Vector3f pos,
                            Eigen::Vector3f vel,
                            Eigen::Vector3f vel_samples)> obstacle_check);
  bool judgeDistance(double current_distance_xy,std::vector<double> &distance_list);
...
private:
  ...
  double factor_;
  double xy_goal_tolerance_limit_current_;
  std::vector<double> distance_store_list50;
};
